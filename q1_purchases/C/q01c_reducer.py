...
text_file = open("q01c_answers.txt", "w")  # text with answers
...
for line in sys.stdin:
    ...
    if current_key == key:  # current_key either None or prev key
        current_max = max(current_max, value)  # max between prev and current SAME KEY!
    else:  # there is a change
        if current_key:            
            # print '%s\t%s' % (current_key, current_max)
            # print(current_key, round(current_max, 2), sep='\t')  # python3 fix
            text_file.write(current_key + '\t' + str(round(current_max, 2)) + '\n')
        current_key = key
        current_max = value
